    ...
    return (
        <div className="dashboard-layout">
            <aside className="dashboard-sidebar">
                <div style={{ padding: "1.25rem 1.25rem 2.5rem", display: "flex", alignItems: "center", gap: "0.85rem" }}>
                    <Link to="/dashboard" style={{ display: "flex", alignItems: "center", gap: "0.85rem" }}>
                        <div style={{ width: "36px", height: "36px", background: "var(--bg-elevated)", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "10px", boxShadow: "0 4px 12px rgba(0,0,0,0.15)", overflow: "hidden", border: "1px solid var(--border-subtle)" }}>
                            <img src="/LOGO.jpeg" alt="Planora Logo" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                        </div>
                        <span style={{ fontSize: "1.25rem", fontWeight: 850, color: "var(--text-primary)", letterSpacing: "-0.03em" }}>Planora</span>
                    </Link>
                </div>
                ...
                <nav style={{ flex: 1, padding: "0 0.75rem" }}>
                    {NAV_ITEMS.map((item) => (
                        ...
                        >
                            <span style={{ display: "flex", alignItems: "center", opacity: location.pathname === item.path ? 1 : 0.65 }}>
                                {item.icon}
                            </span>
                ...
                <div style={{ padding: "0 0.75rem" }}>
                    <div className="user-profile-section" style={{ padding: "1.25rem 0.5rem", borderTop: "1px solid var(--border-subtle)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.85rem", marginBottom: "1rem" }}>
                            <div style={{ width: 38, height: 38, borderRadius: "12px", background: "linear-gradient(135deg, var(--accent-primary) 0%, #2563eb 100%)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, overflow: "hidden", fontSize: "0.95rem", boxShadow: "0 4px 12px rgba(59, 130, 246, 0.25)" }}>
                                {user?.photoURL ? (
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: "0.875rem", fontWeight: 800, color: "var(--text-primary)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", lineHeight: 1.2 }}>
                                    {user?.displayName || "User"}
                                </div>
                                <div style={{ fontSize: "0.725rem", color: "var(--text-muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", marginTop: "0.1rem" }}>
                                    {user?.email}
                                </div>
                            </div>
                        </div>
                        <button onClick={handleLogout} className="sidebar-item" style={{ width: "100%", padding: "0.65rem 0.85rem", justifyContent: "flex-start", color: "#ef4444", border: "1px solid rgba(239, 68, 68, 0.1)", background: "rgba(239, 68, 68, 0.03)", cursor: "pointer", borderRadius: "10px", transition: "all 0.2s" }}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><polyline points="16 17 21 12 16 7" /><line x1="21" y1="12" x2="9" y2="12" /></svg>
                            <span style={{ fontWeight: 750, fontSize: "0.875rem" }}>Sign Out</span>
                        </button>
                    </div>
                </div>
            </aside>
