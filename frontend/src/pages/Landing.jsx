...
export default function Landing() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", color: "#F9FAFB", overflowX: "hidden" }}>
      <Navbar />
      <Hero />
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "4rem 2.5rem 2.5rem" }}>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: "2rem", marginBottom: "3rem" }}>
              <div>
                ...
                <p style={{ fontSize: "0.85rem", color: "#94A3B8", lineHeight: 1.7, maxWidth: 260 }}>
                  Effortlessly manage tasks, set reminders, and stay organized – all in one intuitive platform.
                </p>
              </div>
              {Object.entries(FOOTER_LINKS).map(([heading, links]) => (
                <div key={heading}>
                  <p style={{ fontSize: "0.85rem", fontWeight: 700, color: "#F9FAFB", marginBottom: "1.2rem" }}>
                    {heading}
                  </p>
                  <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.8rem", padding: 0, margin: 0 }}>
                    {links.map((l) => (
                      <li key={l}>
                        <span style={{ fontSize: "0.85rem", color: "#94A3B8", cursor: "pointer" }}>{l}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <p style={{ fontSize: "0.8rem", color: "#64748B" }}>© {new Date().getFullYear()} Planora Technologies.</p>
              <div style={{ display: "flex", gap: "1rem" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748B", cursor: "pointer" }}>Privacy Policy</span>
                <span style={{ fontSize: "0.8rem", color: "#64748B", cursor: "pointer" }}>Terms of Service</span>
              </div>
            </div>
          </div>
        </ClickSpark>
